...
const Vscomputer = ({inputText,val}) => {
    ...
    const {setRound,myTurn}=useCharacterConsumerUpdate();
    const {seconds, loser } = useTimerConsumer()
    const [roundList,setRoundList]=useState([{r1:null},{r2:null},{r3:null},{r4:null},{r5:null}])

    /* if(round===1){
        if(loser.name==='You'){
            setRoundList(pre=>[{...pre,r1:false}])
        }     
        else{
            setRoundList(pre=>[{...pre,r1:true}])
        }    
    }
    if(round===2){
        if(loser.name==='You'){
            setRoundList(pre=>[{...pre,r2:false}])
        }     
        else{
            setRoundList(pre=>[{...pre,r2:true}])
        }    
    }
    if(round===3){
        if(loser.name==='You'){
            setRoundList(pre=>[{...pre,r3:false}])
        }     
        else{
            setRoundList(pre=>[{...pre,r3:true}])
        }    
    } */
    ...
    <div class="container">
        <div class=" row">
            <div class="col-md-8 offset-md-2 ">
               ...
               <ul>
                   <li>
                       <div className= {"cup_icon " + (round === 1 ? '' : (roundList.r1 ? 'bg-success' : 'bg-danger'))}><i className="fa fa-trophy" aria-hidden="true" >{loser.name+round}</i></div>
                   </li>
                   <li>
                       <div className= {"cup_icon " + (round <=2 ? '' : (roundList.r2 ? 'bg-success' : 'bg-danger'))}><i className="fa fa-trophy" aria-hidden="true" >{loser.name+round}</i></div>
                   </li>

                   <li>
                       <div className= {"cup_icon " + (round <=3 ? '' : 'bg-danger')}><i class="fa fa-trophy" aria-hidden="true"></i></div>
                   </li>
                   <li>
                       <div className= {"cup_icon " + (round <=4 ? '' : 'bg-danger')}><i class="fa fa-trophy" aria-hidden="true"></i></div>
                   </li>
                   <li>
                       <div className= {"cup_icon " + (round <=5 ? '' : 'bg-danger')}><i class="fa fa-trophy" aria-hidden="true"></i></div>
                   </li>
                   <div class="clear"></div>
               </ul>
               <div class="clear"></div>
                ...
                <div class="btn_b2">
                    ...
                    <input type='text' placeholder="word" value={inputText} onChange={e=>onChange(e)}></input>
                </div>
